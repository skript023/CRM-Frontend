                ...
                <div class="justify-between mb-6 rounded-lg bg-gray-800 p-6 shadow-md sm:flex sm:justify-start">
                    ...
                    <div class="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                        <div class="mt-5 sm:mt-0">
                            <h2 class="text-lg font-bold text-white">{$selectedProduct?.name}</h2>
                            <p class="mt-1 text-xs text-white">{$selectedProduct?.code}</p>
                        </div>
                        <div class="mt-4 flex justify-between sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
                            <div class="flex items-center border-gray-100">
                                ...
                                <input bind:value={counter} class="h-8 w-8 border bg-white text-center text-xs outline-none" type="number" min="1" />
                            ...
                            <div class="flex items-center space-x-4">
                                <p class="text-sm">IDR {$selectedProduct?.price?.toLocaleString()}</p>
          ...
          <!-- Sub total -->
            <div class="mt-6 h-full rounded-lg border bg-gray-800 p-6 shadow-md md:mt-0 md:w-1/3">
                <div class="mb-2 flex justify-between">
                    ...
                    <p class="text-gray-700">IDR {($selectedProduct?.price * counter)?.toLocaleString()}</p>
                </div>
                <div class="flex justify-between">
                    <p class="text-gray-700">Shipping</p>
                    <p class="text-gray-700">IDR 0</p>
                </div>
                <hr class="my-4" />
                <div class="flex justify-between">
                    <p class="text-lg font-bold">Total</p>
                    <div class="">
                        <p class="mb-1 text-lg font-bold">IDR {($selectedProduct?.price * counter)?.toLocaleString()}</p>
